  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        sticky ? "bg-white shadow-md" : "bg-white"
      }`}
    >
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 md:px-12 lg:px-16 xl:px-20 2xl:px-32">
        <div className="flex items-center justify-between h-16 min-w-0">
          ...
          <Link to="/" className="flex items-center gap-3 min-w-0 shrink-0">
            ...
            <span
              className={`font-bold tracking-wide leading-tight text-sm sm:text-base md:text-lg lg:text-lg xl:text-xl 2xl:text-2xl whitespace-nowrap truncate ${
                sticky ? "text-blue-900" : "text-blue-900"
              }
`}
            >
 ...
 ${isActive ? activeClass : ""}`
          ...
          <div className="hidden xl:flex items-center gap-3 flex-shrink-0">
            {socialLinks.map((link, index) => (
              <a
                ...
                className={`${link.style} text-lg transition`}
              >
            ...
            <a
              ...
              target="_blank"
              className="ml-3 bg-orange-400 text-white text-sm px-4 py-1.5 rounded-md hover:bg-orange-600 transition"
            >
              Connect
            </a>
          </div>
          ...
          <div className="xl:hidden flex-shrink-0">
            ...
            >
              <svg
                ...
                className={`h-6 w-6 ${sticky ? "text-black" : "text-black"}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
      ...
      <div
        className={`xl:hidden fixed inset-0 bg-white z-40 flex flex-col items-center justify-center gap-8 text-xl transition-transform duration-300 ${
          isMobileMenuOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <button
          onClick={() => setIsMobileMenuOpen(false)}
          className="absolute top-5 right-5 p-2 text-2xl font-bold"
        >
          ✕
        </button>
